...
define('JOB_INTERLEAVE', 10); // in seconds

require(dirname(dirname(dirname(dirname(__FILE__)))).'/config.php'); // Global moodle config file.
require_once($CFG->dirroot.'/lib/clilib.php'); // CLI only functions

mtrace("CasENT Automated Distributed Fixture Tool");
mtrace("##########################################\n");
...
if ($options['help']) {
    $help =
        ...
        -f, --fixture       CLI Script name to give to workers.
...
if (!file_exists($CFG->dirroot.'/auth/casent/cli/'.$options['fixture'].'.php')) {
...
foreach ($joblist as $jl) {
    ...
    if (!empty($jl)) {
        ...
        $hids = implode(',', $jl);
        $workercmd = "$phpcmd \"{$CFG->dirroot}/auth/casent/cli/fixture_worker.php\" --nodes=\"$hids\" $logattr --fixture={$options['fixture']} $verboseattr";
